...
- `GMAIL_CLIENT_ID` + `GMAIL_CLIENT_SECRET` + `GMAIL_REFRESH_TOKEN` — send verification to the signup email via Gmail (`hello@isexperience.house`)
- `MAIL_RELAY_URL` + `MAIL_RELAY_SECRET` — optional Apps Script relay
- `RESEND_API_KEY` + `MAIL_FROM` — Resend fallback (test mode only emails the Resend account owner)
